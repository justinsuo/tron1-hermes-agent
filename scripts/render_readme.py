#!/usr/bin/env python3
"""Regenerate ~/tron1-hermes-agent/README.md with the latest live stats.
Invoked by sync_to_repo.sh on every auto-push cycle.

The README intentionally embeds running numbers (episodes, success rate,
gauge readings, last-sync time) so the GitHub front page is a live
dashboard rather than a static brochure.
"""
...
ranked = sorted(per_task.items(), key=lambda kv: kv[1].get("rate_pct", 0), reverse=True)
task_rows = []
...
if live_p.exists():
    try:
        ...
        if p:
            bits.append(
                f"**pose** `({p.get('x',0):+.2f}, {p.get('y',0):+.2f}, "
                f"yaw={p.get('yaw',0):+.2f})`"
            )
        if g:
            bits.append(
                "**gauges** "
...
**{total} total episodes · {rate:.0f}% success on the most recent 30**

{live_text or "_(no live state snapshot yet — sim probably not running)_"}
